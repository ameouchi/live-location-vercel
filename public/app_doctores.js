...
map.on('load', async () => {
  ...
  const bedrock = await loadGeo('acequia_doctores.geojson');
  ...
  map.addLayer({
    ...
    source: 'live-head',
    paint: {
      'circle-radius': ['interpolate', ['linear'], ['get','pulse'], 0, 4, 1, 30],
      'circle-color': '#ffffff',
...
async function addGeoTiffLayer(){
  ...
  try{
    ...
    function setGeoBtnText(){
      ...
      btn.textContent = (vis === 'visible') ? 'NO VER GEOLOGIA' : 'VER GEOLOGIA';
    }
    if (btn){
      btn.onclick = () => {
        const vis = map.getLayoutProperty(layerId, 'visibility') || 'visible';
